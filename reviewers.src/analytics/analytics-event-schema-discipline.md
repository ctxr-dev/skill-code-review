...
type: primary
focus: Detect analytics event-schema gaps including inconsistent naming conventions, no event registry,
  PII in event properties, high-cardinality dimensions, unversioned payload changes, duplicated events,
  missing identify-on-auth anonymous stitching, and third-party sends without contract review
parents:
- index.md
covers:
