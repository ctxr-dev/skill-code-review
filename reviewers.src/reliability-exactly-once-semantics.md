...
type: primary
focus: Detect at-most-once where at-least-once is needed, at-least-once without idempotent consumers,
  and exactly-once claims without transactional outbox
parents:
- index.md
aliases:
- pattern-outbox
...
- Outbox relay that does not handle duplicates forcing downstream consumers to deal with at-least-once
  delivery
- Outbox table without cleanup or retention policy leading to unbounded table growth
- Missing outbox where a service writes to its database and publishes an event non-atomically (dual-write
  problem)
