...
* :mod:`ctxr_skill_code_review.spec` — the Pydantic :class:`~ctxr.fsm.FsmSpec`
  literal that mirrors the YAML 1:1 (15 states, 8 worker schemas, 9 inline
  states, 1 terminal).
* :mod:`ctxr_skill_code_review.handlers` — the 9 deterministic Python
  callables that run inside :func:`ctxr.fsm.execute_inline`.
* :mod:`ctxr_skill_code_review.workers` — the 5 worker prompt `.md`
  templates the FSM hands to dispatched sub-agents.
* :mod:`ctxr_skill_code_review.install` — the one-shot
  :func:`~ctxr_skill_code_review.install.register` function that wires
...
# ``python -m ctxr_skill_code_review.install`` does NOT trigger the
...
if TYPE_CHECKING:
    ...
    def __getattr__(name: str) -> Any:
        if name == "register":
            from ctxr_skill_code_review.install import register as _register
