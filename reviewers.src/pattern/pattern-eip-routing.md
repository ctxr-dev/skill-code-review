...
type: primary
focus: Detect misuse, absence, and over-engineering of Enterprise Integration message routing patterns
  -- content-based routing, splitting, aggregating, and scatter-gather.
parents:
- index.md
covers:
...
- Every router has a default or fallback route that handles unrecognized message types explicitly (error,
  DLQ, or log)
- Router logic is extensible without modification -- new message types can be added via configuration,
  registry, or handler registration
...
- Aggregators have a completion condition (count, timeout, or sentinel) and do not accumulate fragments
  forever
...
- Router performance is acceptable -- routing decisions do not require expensive I/O or computation per
  message
languages:
- cpp
- csharp
