...
- correctness
audit_surface:
- Every decorator implements exactly the same interface as the component it wraps -- no added or removed
  methods
- Decorator delegates to the wrapped component for all interface methods, not just some
- Decorator does not silently alter return values, suppress exceptions, or change observable behavior
...
- Decorator does not swallow exceptions from the delegate -- it may translate or wrap them but must propagate
  failure
- All interface methods are implemented after interface evolution -- no default/empty stubs hiding missing
  delegation
- Decorator does not conditionally skip delegation entirely (short-circuit) without clear documentation
- Identity operations (equals, hashCode, toString, compareTo) either delegate to the wrapped component
  or are explicitly overridden with documented semantics
...
- 'Decorator is transparent: callers cannot distinguish a decorated component from an undecorated one
  (Liskov Substitution)'
languages:
- cpp
- csharp
