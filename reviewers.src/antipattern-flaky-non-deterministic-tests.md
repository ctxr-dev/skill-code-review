...
type: primary
focus: Detect tests that pass or fail unpredictably due to hidden dependencies on time, ordering, network,
  shared state, or randomness
parents:
- index.md
covers:
- Tests depending on wall-clock time (Date.now, time.time, System.currentTimeMillis) without freezing
  or mocking the clock
...
- correctness
audit_surface:
- Test calls Date.now(), time.time(), System.currentTimeMillis(), Instant.now(), or DateTime.UtcNow without
  a mocked or frozen clock
...
- Test asserts on error message strings, log output, or exception text that is not part of the public
  API
