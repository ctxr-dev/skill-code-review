...
type: primary
focus: Detect ASP.NET Core and Blazor pitfalls including missing authorization attributes, input validation
  gaps, Blazor Server circuit state leaks, IJSRuntime XSS, anti-forgery token omissions, CORS misconfiguration,
  connection string exposure, and missing exception handling middleware that cause vulnerabilities or
  production failures.
parents:
- index.md
covers:
