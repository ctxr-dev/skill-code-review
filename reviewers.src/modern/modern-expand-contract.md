...
type: primary
focus: Detect expand-contract (parallel change) violations where the contract is broken during migration,
  old consumers are not migrated, new fields lack backfill, the expand phase ships without population,
  or old fields are removed prematurely
parents:
- index.md
aliases:
