...
- correctness
audit_surface:
- No strong reference cycles — verify `weak`/`unsafe_unretained` on delegate properties, block captures,
  timers
- Block capture list uses weakSelf/strongSelf dance correctly
- 'Properties use correct attribute: `copy` for value-semantic types (NSString, NSArray, blocks), `weak`
  for delegates'
