...
type: primary
focus: Detect runtime-incompatible APIs, overbroad permissions, cold-start blindspots, and unpinned deps
  across Deno, Bun, and Node.js (including edge/isolate deployments)
parents:
- index.md
covers:
...
- performance
audit_surface:
- Code uses Node-only APIs (`fs`, `child_process`, `cluster`, `net`, `tls`, `dgram`, `worker_threads`)
  in an edge-runtime entrypoint
