...
id: fw-htmx
type: primary
focus: Detect htmx-specific pitfalls in user-controlled URLs, HTML fragment XSS, missing CSRF tokens,
  unsafe swap modes, missing confirmation on destructive actions, URL manipulation via hx-push-url, server
  endpoint design, and polling without rate limits.
parents:
- index.md
covers:
