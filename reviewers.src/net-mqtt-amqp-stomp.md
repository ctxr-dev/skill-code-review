...
type: primary
focus: Detect messaging protocol issues including MQTT without TLS, incorrect QoS for critical messages,
  missing dead-letter exchanges, topic ACL gaps, and consumer acknowledgment misuse
parents:
- index.md
aliases:
...
- Payload serialization format chosen without considering schema evolution (no Avro, Protobuf, or JSON
  Schema)
...
- Consumer that acknowledges messages before processing completes (at-most-once when at-least-once is
  needed)
