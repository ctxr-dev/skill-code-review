...
- **Dispatching more than 10 specialist Agents in one orchestrator message** — never. The runner caps `--print-pending-leaf-ids` at 10 ids per call (override with `--batch-size N`, max 50). The thread-pool model exists to keep observability tight: 30 simultaneous Agent dispatches in one message overflows the trace window and makes failures hard to attribute. Batches of 10 are observable, debuggable, and produce identical findings to one big batch.
