...
type: primary
focus: Detect application services containing domain logic, application services directly accessing infrastructure,
  and application services performing orchestration that belongs in domain services.
parents:
- index.md
...
- Application service directly calling infrastructure (database, HTTP, file system) without going through
  ports
...
- Application service performing calculations (totals, discounts, eligibility) instead of delegating to
  domain
...
- Application service with methods named using domain verbs (approve, escalate) that contain the logic
