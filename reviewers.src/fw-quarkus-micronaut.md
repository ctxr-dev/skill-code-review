...
type: primary
focus: Detect Quarkus and Micronaut pitfalls including GraalVM native-image reflection breakage, CDI scope
  misuse, event-loop blocking, missing health checks, and serialization failures that cause build-time
  or runtime errors invisible during JVM development.
parents:
- index.md
covers:
