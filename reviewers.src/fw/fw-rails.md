...
id: fw-rails
type: primary
focus: Detect Ruby on Rails pitfalls in mass assignment, CSRF bypass, controller authorization, SQL injection
  via string interpolation, open redirects, path traversal, callback coupling, and N+1 queries that cause
  vulnerabilities or production failures.
parents:
- index.md
aliases:
