...
type: primary
focus: Detect use of a familiar technology, pattern, or tool for every problem regardless of whether it
  fits the specific requirements
parents:
- index.md
covers:
...
- Same language or framework used for workloads it handles poorly (Python for CPU-bound without native
  extensions)
...
- Full DDD tactical patterns (aggregates, repositories, domain events) for a CRUD endpoint with no business
  rules
dimensions:
- architecture
- readability
