...
type: primary
focus: Detect Svelte 5 and SvelteKit pitfalls in reactivity, lifecycle, form handling, and server/client
  boundaries that cause bugs, XSS, or incorrect rendering.
parents:
- index.md
covers:
...
- security
audit_surface:
- $effect() callback that registers addEventListener, setInterval, or external subscription without returning
  a cleanup function
...
- '{@html variable} where variable traces to user input, API response, URL parameter, or any external
  source'
