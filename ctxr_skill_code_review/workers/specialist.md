...
2. For each check, scan the filtered diff for evidence. Trace each changed line with these **bug-hunting heuristics** (apply the ones in your lane):
   ...
   - **Unset/missing-state (high-value, often missed):** when the change reads a value from a place that is NOT guaranteed to have been populated on every path — session/pipeline/request state, a cache entry, an optional config key, a prior step's output, a dict key set only in some branches — ask "is there a path where this is absent/None, and is it used here WITHOUT a guard?" An unchecked read of a maybe-missing value is a concrete null-deref / `KeyError` / `NoneType` bug. Trace the producing path to confirm, then flag it. **Emit ONE finding per DISTINCT unsafe access, even when several sit in the same function/region: a `None`-deref on value A (e.g. `fetch_state("x")` returning None used without a check) and a `KeyError` on value B (e.g. `obj["k"]["sub"]` when `k` may be absent) are TWO separate bugs with two separate fixes — report both, do not bundle them into one finding. Bundling distinct root causes hides one of them from the reader and the ranker.**
