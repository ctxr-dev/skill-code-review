...
type: primary
focus: Detect misuse, over-application, and absence of the Abstract Factory pattern when creating families
  of related objects.
parents:
- index.md
aliases:
...
- architecture
audit_surface:
- All products created by a single abstract factory belong to one cohesive family (UI kit, database suite,
  protocol stack)
...
- The abstract factory interface is narrow enough that adding a new product does not force all concrete
  factories to change
...
- Platform/theme/environment selection is centralized in factory selection logic, not scattered across
  business code
