...
type: primary
focus: Detect Laravel and Symfony pitfalls including Blade/Twig XSS via unescaped output, mass assignment
  without fillable/guarded, missing CSRF tokens, raw SQL injection, missing middleware, debug mode exposure,
  Eloquent N+1 queries, and missing authorization that cause vulnerabilities or production failures.
parents:
- index.md
covers:
