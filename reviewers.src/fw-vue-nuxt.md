...
id: fw-vue-nuxt
type: primary
focus: Detect Vue 3 and Nuxt 3 pitfalls in reactivity, component design, composables, and server-side
  rendering that cause subtle bugs, XSS, or performance degradation.
parents:
- index.md
covers:
...
- onMounted callback registering addEventListener, setInterval, or external subscriptions with no onUnmounted
  cleanup
...
- watch() or watchEffect() used where a computed property would express the same derivation without side
  effects
