...
id: lang-scala
type: primary
focus: Sealed ADTs, Option/Either discipline, effect systems (Cats Effect, ZIO), implicits/givens, and
  immutable-first Scala idioms
parents:
- index.md
covers:
...
- correctness
audit_surface:
- No `.get` on `Option`, `Try`, or `Either` in production — use `fold`, `map`, `getOrElse`, or pattern
  match
...
- '`NonFatal` guard in every `catch` block — `OOM`, `StackOverflowError`, `InterruptedException` never
  caught'
- '`ExecutionContext` explicitly provided to `Future` — `scala.concurrent.ExecutionContext.global` not
