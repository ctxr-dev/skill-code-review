...
id: fw-vapor-swift
type: primary
focus: Detect Vapor (Swift) pitfalls in Content validation, Fluent authorization, middleware configuration,
  async blocking, CSRF protection, secrets management, response data leakage, and error handling that
  cause security vulnerabilities or runtime failures.
parents:
- index.md
