...
- **Wiki-routed expertise.** A corpus of 479 leaf reviewers (`reviewers.wiki/`),
...
findings/PR), not by being conservative. This skill targets the same frontier: on a
5-PR pilot its `primary` set reaches **recall 0.73 / precision 0.53 / F1 ~0.62**
with **~1.4 false positives per PR**, matching Cubic's full-50 frontier on F1 while
staying quiet. Much of the residual gap to Cubic is the skill surfacing *real bugs
the golden set simply does not list* (the harsh metric penalises thoroughness). A
full-50 head-to-head is the definitive test and the next milestone; the
optimization loop that drives it is the
...
file by specialist coverage matrix. Schema: [`report-format.md`](report-format.md).
