...
type: primary
focus: Detect misuse, deadlock risk, and absence of the Monitor Object pattern in synchronized shared-state
  code.
parents:
- index.md
covers:
...
- No synchronized method performs blocking I/O, network calls, or long computations while holding the
  lock
- Nested locking across multiple monitors uses a consistent, documented lock ordering
- Shared mutable state is fully encapsulated within a monitor, not protected by ad-hoc `synchronized`
  blocks scattered across unrelated classes
- '`notify()` is used only when exactly one waiter exists and all waiters have identical conditions; otherwise
  `notifyAll()` is used'
...
- Synchronized methods do not return mutable references to internal state (breaks encapsulation of the
  monitor)
