...
type: primary
focus: Detect methods that use more fields and methods of another class than their own, indicating misplaced
  logic.
parents:
- index.md
covers:
...
- Service method extracting multiple properties from a domain object to compute a result the domain object
  should own
...
- Method accessing nested getters (order.getCustomer().getAddress().getCity()) to gather data for a local
  computation
