...
type: primary
focus: Detect path traversal vulnerabilities and insecure file upload handling that enable unauthorized
  file access, code execution, or denial of service
parents:
- index.md
covers:
...
- Path.join(), os.path.join(), or path.resolve() with user-controlled segments without post-join canonicalization
  check
