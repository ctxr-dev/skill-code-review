...
type: primary
focus: Detect domain concepts represented as raw primitive types instead of expressive value objects or
  domain types
parents:
- index.md
covers:
...
- architecture
audit_surface:
- Parameter typed as string/int that represents a domain concept (email, currency code, URL, phone number,
  postal code)
...
- Variable named with a unit suffix (priceInCents, durationMs, distanceKm) compensating for the type's
  lack of unit information
