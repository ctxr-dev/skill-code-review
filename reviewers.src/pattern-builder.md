...
- correctness
audit_surface:
- 'Builder is justified: the product has 4+ fields, or requires complex assembly, or has many optional
  parameters'
...
- Builder is co-located with or has appropriate access to the product (not reaching into private state
  from another package)
- Director class, if present, is justified by multiple build sequences -- not just wrapping a single build
  flow
languages:
- cpp
- csharp
