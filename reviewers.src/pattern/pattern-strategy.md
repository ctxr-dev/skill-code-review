...
- architecture
audit_surface:
- Strategy interface has more than one concrete implementation, or a second implementation is clearly
  anticipated
- Strategy is injected into the context (constructor, setter, method parameter) rather than selected by
  if/else or switch inside the context
- Strategy implementation depends only on data passed through the strategy method parameters, not on context
  internals
- Strategies are stateless or manage their own isolated state -- no shared mutable state via context
- Duplicated conditional logic across call sites has been consolidated into a strategy or extracted behind
  a factory
