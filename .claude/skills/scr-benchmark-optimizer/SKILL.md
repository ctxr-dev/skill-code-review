...
description: >
  ...
  open Greptile/Martian set and apply proven mechanical optimizations until it
  sits on the precision-recall frontier (high coverage AND low noise). This is a
...
ThreadPoolExecutor parallelism, deterministic collection + deduper agent, fault
tolerance, embeddings, human-gated wiki changes), the harness layout under
`ctxr-dev/tmp/`, and the current standing vs competitors.
