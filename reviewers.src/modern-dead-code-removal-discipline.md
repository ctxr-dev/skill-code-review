...
type: primary
focus: Detect dead code removal discipline failures where identified dead code is not removed promptly,
  removal is mixed with feature changes, feature flags are not cleaned up, dead dependencies linger, unused
  database artifacts remain, or commented-out code is preserved
parents:
- index.md
aliases:
