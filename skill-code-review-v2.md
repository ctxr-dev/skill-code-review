...
|--------|----------|------|------------------------------|
| **Sprint A** — FSM consumer wiring | **P0** | Author `fsm/code-reviewer.fsm.yaml` + skill-specific worker prompts under `fsm/workers/`. Add `@ctxr/fsm` as a `file:../fsm` dependency. Wire the package's CLIs (`fsm-validate-static`, `fsm-next`, `fsm-commit`, `fsm-inspect`) into `package.json` scripts. Add `.fsmrc.json` setting `fsm_path` and `storage_root: .skill-code-review`. Engine itself ships from `@ctxr/fsm`; design + reference docs live there too. | None — depends only on the FSM package |
