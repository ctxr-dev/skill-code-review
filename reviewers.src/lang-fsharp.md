...
- Computation expressions use correct `let!`/`return`/`do!` — no accidental double-wrapping (e.g., `async
  { return! async { ... } }` instead of flattening)
...
- '`Seq` (lazy) vs `List` (eager) vs `Array` (perf) chosen deliberately — `Seq` not iterated multiple
  times'
...
- C# interop boundary handles nulls with `Option.ofObj`/`Option.toObj` — no `NullReferenceException` leaking
  in
languages:
- fsharp
tags:
