...
id: pattern-command
type: primary
focus: Detect misuse, over-application, and absence of the Command pattern in operation-encapsulation
  code.
parents:
- index.md
covers:
...
- architecture
audit_surface:
- Command constructor captures parameters and receiver references only -- no side effects, I/O, or validation
  occur until execute()
...
- Operations that need queuing, scheduling, or undo are encapsulated as command objects, not raw method
  calls
...
- Command captures only the data needed for execution, not large state that could be resolved at execution
  time
- Each command encapsulates a single cohesive operation
- Command history (undo stack) has a bounded size with eviction policy
- Parameters are validated before side effects begin -- execute() does not leave partial state on validation
  failure
