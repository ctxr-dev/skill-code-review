...
- '`struct` field typed as `Any`, `Real`, `AbstractArray`, or other abstract type — prevents efficient
  memory layout'
...
- '`@inbounds` applied without prior bounds validation — out-of-bounds is undefined behavior, not a catchable
  error'
...
- 'Method ambiguity exists: `f(::A, ::B)` and `f(::C, ::D)` where `A <: C` and `D <: B` with no `f(::A,
  ::D)` defined'
