...
id: fw-ktor
type: primary
focus: Detect Ktor-specific pitfalls including missing plugins for content negotiation, error handling,
  and authentication, coroutine scope misuse, blocking calls without dispatcher switch, and unvalidated
  request input that cause silent failures or security holes.
parents:
- index.md
covers:
