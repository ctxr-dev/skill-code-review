...
type: primary
focus: Detect branch-by-abstraction failures where the abstraction layer is missing before an implementation
  swap, old implementations linger after stabilization, abstractions leak details, or feature toggles
  are absent for switching
parents:
- index.md
aliases:
- pattern-bridge
...
- Interface or abstract class shaped by the old implementation's method signatures rather than domain
  semantics
