...
- correctness
audit_surface:
- Every `try` expression is in a function whose return type includes the error set, or is inside a `catch`
  block
- '`defer` and `errdefer` properly pair with every resource acquisition (allocator.alloc, fopen, mutex
  lock)'
