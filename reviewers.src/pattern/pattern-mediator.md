...
id: pattern-mediator
type: primary
focus: Detect misuse, over-application, and absence of the Mediator pattern in inter-object coordination
  code.
parents:
- index.md
covers:
...
- Mediator is stateless or holds only transient routing state -- it does not cache or accumulate colleague
  data
...
- 'Coupling between mediator and colleagues is unidirectional: colleagues know the mediator interface,
  mediator knows colleague interfaces, but not the reverse at the concrete level'
