...
id: pattern-observer
type: primary
focus: Detect misuse, over-application, and absence of the Observer pattern in event-driven notification
  code.
parents:
- index.md
covers:
...
- correctness
audit_surface:
- Every subscribe/on/addEventListener has a corresponding unsubscribe/off/removeEventListener on the appropriate
  lifecycle boundary
...
- Subject's observer list is thread-safe if subscribe, unsubscribe, or notify can be called from multiple
  threads
...
- Observer registration and deregistration are idempotent (double-subscribe or double-unsubscribe do not
  corrupt state)
languages:
- cpp
- csharp
