...
type: primary
focus: Detect missing failure injection for critical dependencies, absent steady-state hypotheses, uncontrolled
  blast radius, and missing rollback plans
parents:
- index.md
covers:
- Critical dependency without failure injection experiments (database, cache, message broker, third-party
  API)
